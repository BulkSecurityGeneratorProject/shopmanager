...
@Component({
    selector: 'jhi-product-client-shp-mng',
    templateUrl: './product-client-shp-mng.component.html'
})
export class ProductClientShpMngComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/product-client-shp-mng'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/product-client-shp-mng', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProductClientShpMng) {
        return item.id;
    }
