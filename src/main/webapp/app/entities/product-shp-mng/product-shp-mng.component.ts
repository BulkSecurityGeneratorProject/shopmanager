...
@Component({
    selector: 'jhi-product-shp-mng',
    templateUrl: './product-shp-mng.component.html'
})
export class ProductShpMngComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/product-shp-mng'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/product-shp-mng', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProductShpMng) {
        return item.id;
    }
